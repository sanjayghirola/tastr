  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 p-4">
        ...
        {/* Pickup PIN — visible once order is accepted */}
        {order.pickupPin && ['ACCEPTED','PREPARING','READY','DRIVER_ASSIGNED'].includes(order.status) && (
          <div className="bg-amber-50 border-2 border-amber-300 rounded-xl px-3 py-2 text-center shrink-0">
            <p className="text-[9px] font-bold text-amber-600 uppercase tracking-widest">Pickup PIN</p>
            <p className="text-lg font-black text-amber-800 tracking-[0.2em] font-mono">{order.pickupPin}</p>
          </div>
        )}
      ...
      <div className="fixed inset-0 z-40 flex" onClick={onClose}>
        ...
        <div className="w-full max-w-xl bg-white h-full overflow-y-auto shadow-2xl border-l border-border"
          ...
          <div className="p-6 space-y-6">
            ...
            {/* Pickup PIN for driver verification */}
            {order.pickupPin && !['CANCELLED','REJECTED','FAILED','DELIVERED'].includes(s) && (
              <div className="bg-gradient-to-r from-amber-50 to-orange-50 border-2 border-amber-300 rounded-2xl p-5 text-center">
                <div className="flex items-center justify-center gap-2 mb-2">
                  ...
                  <p className="text-xs font-bold text-amber-700 uppercase tracking-widest">Driver Pickup PIN</p>
                </div>
                <p className="text-4xl font-black text-amber-900 tracking-[0.3em] font-mono my-2">{order.pickupPin}</p>
                <p className="text-xs text-amber-600">
                  Show this PIN to the delivery driver when they arrive to collect the order.
                  The driver must enter this PIN to confirm pickup.
                </p>
              </div>
            )}
